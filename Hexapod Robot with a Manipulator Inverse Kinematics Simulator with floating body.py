...
# -----------------------------
# COMPUTE NATURAL FEET
# -----------------------------
feet_world = {}
for leg, q in default_leg_angles.items():
    hip = hip_offsets[leg]
    feet_world[leg] = hip + fk_leg(*q)

# -----------------------------
# ARM
# -----------------------------
arm_offset = np.array([0.0, 0.22, 0.0])
arm_target = np.array([0.0, 0.28, 0.1])
arm_default = arm_target.copy()

# MODIFICATION: Set theta3 (index 2) to -1.0 to favor Elbow-Up on start
...
# -----------------------------
# IK LEG (MODIFIED to accept elbow toggle)
# -----------------------------
...
def draw():
    ...
    for leg in feet_world:
        ...
        # MODIFICATION: Flip elbow (theta3) for right-side legs
        is_right = leg in ["FR", "MR", "RR"]
        q = ik_leg(*foot_local, flip_elbow=is_right)
...
# -----------------------------
root = tk.Tk()
root.title("Hexapod Floating Base IK + Arm (Natural Stance)")
...
limits = [(-0.2, 0.2), (-0.2, 0.2), (-0.35, 0.05),
...
def reset():
    ...
    # MODIFICATION: Reset theta3 (index 2) to -1.0 here as well
    prev_arm_q = np.array([0.0, 0.0, -1.0])
